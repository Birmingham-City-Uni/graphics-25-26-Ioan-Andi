...
// ***** WEEK 8 LAB *****
// This file implements Task 2 of the rasterisation lab.
//
// Goal:
// Extend the rasteriser to support more accurate shading and compare
// Phong and Blinn-Phong specular models.
//
// Main additions in this task:
// 1. Implement the Blinn-Phong specular term in Shading.hpp.
// 2. Use perspective-correct interpolation for fragment attributes.
// 3. Compare the visual behaviour of Phong and Blinn-Phong highlights.
...
{
	// Construct a perspective projection matrix using the horizontal
	// field of view and the image aspect ratio.
...
void findScreenBoundingBox(const Triangle& t, int width, int height, int& minX, int& minY, int& maxX, int& maxY)
{
	...
	// Restricting rasterisation to this region reduces the number of pixels
	...
	// Clamp the bounding box so it remains within the image boundaries.
	...
	const Eigen::Vector3f& camWorldPos)
{
	...
	if (triangleArea < 0) {
		// Back-face culling: skip triangles whose screen-space winding
		// indicates they face away from the camera.
		return;
	}
	...
	for (int x = minX; x <= maxX; ++x)
		for (int y = minY; y <= maxY; ++y) {
			...
			// Compute the areas of the three sub-triangles formed with the
			// current sample point. These are used to derive barycentric weights.
			...
			// Reject points outside the triangle.
			float sum = b0 + b1 + b2;
			if (sum > 1.0001f) {
				continue;
			}

			// Task 2 uses perspective-correct interpolation. Screen-space
			// barycentric coordinates are first divided by depth, then
			// renormalised before being used to interpolate world-space data.
			float w0 = b0 / t.screen[0].z();
			float w1 = b1 / t.screen[1].z();
			float w2 = b2 / t.screen[2].z();
			float wSum = w0 + w1 + w2;
			...
			// Interpolate the fragment position in world space using the
			// perspective-correct weights.
			Eigen::Vector3f worldP = t.verts[0] * w0 + t.verts[1] * w1 + t.verts[2] * w2;

			// Interpolate depth for z-buffer visibility testing.
			...
			// Interpolate and normalise the surface normal using the
			// perspective-correct weights.
			Eigen::Vector3f normP = t.norms[0] * w0 + t.norms[1] * w1 + t.norms[2] * w2;
			normP.normalize();

			// Accumulate illumination from all light sources.
			Eigen::Vector3f color = Eigen::Vector3f::Zero();

			for (auto& light : lights) {

				// Evaluate the intensity of the current light at the fragment position.
				...
				if (light->getType() != Light::Type::AMBIENT) {
					...
					// Compute the viewing direction from the fragment toward the camera.
					Eigen::Vector3f viewDir = (camWorldPos - worldP).normalized();

					// Evaluate the Blinn-Phong specular term. This uses the
					// half-vector between the light and view directions rather
					// than an explicitly reflected vector.
					...
					// Compute the Lambertian diffuse term using the angle between
					// the normal and the outgoing light direction.
					float dotProd = normP.dot(-incomingLightDir);

					// Clamp negative values so that only front-facing illumination contributes.
					...
					// Ambient light contributes uniformly across the surface and
					// is modulated only by the material albedo.
	...
	int width, int height)
{
	for (int i = 0; i < mesh.vFaces.size(); ++i) {
		...
		// Transform vertices into clip space and perform the perspective divide
		...
		// Map normalised device coordinates from [-1, 1] into image-space coordinates.
		...
		// the upper-left 3x3 portion of the model matrix.
...
int main()
{
	...
	// Initialise the image to black and set the z-buffer to the farthest depth.
	...
	// Place the camera slightly above the scene and tilt it downward.
	...
	// Define the scene lighting configuration.
	...
	// Render the bunny using Blinn-Phong shading.
	...
	// Draw point-light markers to support debugging and scene inspection.
	...
	// Save a visualisation of the z-buffer for debugging.
